...
- **This machine needs `PG_ALLOW_INSECURE_TLS=1` set to reach the test database** — a documented, pre-existing condition (`packages/database/src/postgresClient.ts`'s `isLocalHost`/`allowInsecureTls` comment): Supabase's pooler cert trips `SELF_SIGNED_CERT_IN_CHAIN` on this machine specifically, and the flag is this codebase's own narrow, explicit opt-in for it (relaxes only certificate-identity verification, not encryption). Prefix every command that touches the database: `PG_ALLOW_INSECURE_TLS=1 pnpm build && PG_ALLOW_INSECURE_TLS=1 pnpm test:integration -- <name>`. Confirmed against a clean 71/71-passing baseline run with the flag set before any task in this plan started.
