...
// `?path=/` (Manager overview), `?path=/employees…`, `?path=/supervisors`, `?path=/admins` and `?path=/recent-activity` run against the handoff's own morning; everything else is the schedule week.
const path = params.get('path') ?? '';
export const callRpc =
  path === '/' || path.startsWith('/employees') || path.startsWith('/supervisors') || path.startsWith('/admins') || path.startsWith('/recent-activity')
