...
export const supervisorTableRow = (row: SupervisorRow): PeopleTableRow => ({
  ...row,
  cells: [row.department, row.permissions, row.teamSize],
});
...
const TABLE_GRID =
  'grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)_minmax(0,0.9fr)_minmax(0,1fr)_minmax(70px,auto)]';
const CELL = 'min-w-0 truncate text-[12.5px] text-[#857A72]';

export const pillStyle = (tone: Tone): React.CSSProperties => ({
  color: TONES[tone][0],
  backgroundColor: TONES[tone][1],
});
      ...
      <section className="overflow-hidden rounded-[16px] border border-solid border-[#EBE7E3] bg-white">
        <div className="overflow-x-auto">
          <div className="min-w-[760px]">
            <div
              className={`grid ${grid} gap-3 border-b border-solid border-[#F2EEEA] px-[18px] py-[11px] text-[10.5px] font-extrabold uppercase tracking-[.08em] text-[#A79C93]`}
            >
              {columns.map((column, index) => (
                <span
                  key={column}
                  className={
                    index === columns.length - 1 ? 'min-w-0 text-right' : 'min-w-0 truncate'
                  }
                >
                  {column}
                </span>
            ...
            {rows.length === 0 ? (
              <p className="m-0 border-b border-solid border-[#F7F4F1] px-[18px] py-3.5 text-[12.5px] text-[#A79C93]">
                {emptyLine}
              </p>
            ) : (
              rows.map((row) => (
                <div
                  key={row.id}
                  className={`grid ${grid} items-center gap-3 border-b border-solid border-[#F7F4F1] px-[18px] py-3`}
                >
                  <span className="flex min-w-0 items-center gap-[11px]">
                    <span
                      className="flex size-[30px] flex-none items-center justify-center rounded-full text-[10.5px] font-extrabold"
                      style={avatarTone(row.name)}
                    >
                      {initialsOf(row.name)}
                    </span>
...
/** The handoff's shared list toolbar (markup lines 363-376): search, filter buttons and the result count. */
export function ListToolbar<F extends string>({
  filters,
  filter,
  onFilter,
  query,
      ...
      {filters.map((name) => (
        <button
          ...
          className={[
            ...
            filter === name
              ? 'border-[#F04E17] bg-[#FDF0E9] text-[#C6420E]'
              : 'border-[#EBE7E3] bg-white text-[#857A72]',
          ].join(' ')}
        >
          {name}
        </button>
