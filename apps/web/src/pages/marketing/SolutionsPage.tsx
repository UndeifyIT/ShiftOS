  ...
  return (
    <MarketingLayout>
      <div className="mx-auto max-w-7xl px-4 pb-16 pt-13 sm:px-6">
          ...
          {PROBLEMS.map((p) => (
            <article key={p.problem} className="flex flex-wrap overflow-hidden rounded-2xl border border-neutral-200 bg-white">
              <div className="min-w-[260px] flex-1 basis-[300px] border-b border-neutral-100 bg-[#FDFCFB] p-5.5 sm:border-b-0 sm:border-r">
                ...
                <p className="mt-2 text-base text-neutral-600">{p.pain}</p>
              </div>
              <div className="min-w-[260px] flex-1 basis-[320px] p-5.5">
                <p className="text-[12.5px] font-extrabold uppercase tracking-[0.1em] text-success-text">In ShiftOS</p>
                <p className="mt-2 text-base font-bold text-neutral-900">{p.fix}</p>
                <p className="mt-2 text-base text-neutral-600">{p.detail}</p>
              </div>
            </article>
          ))}
        </div>
        ...
        <p className="mt-2 text-base text-neutral-600">The same operating model fits any business where people rotate through a location.</p>
        <div className="mt-4.5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {INDUSTRIES.map((i) => (
            <div key={i.name} className="rounded-xl border border-neutral-200 bg-white p-4">
              ...
              <p className="mt-1 text-sm text-neutral-600">{i.note}</p>
            </div>
          ))}
        </div>
      </div>
