      ...
      <div className="mx-auto max-w-7xl px-4 pb-16 pt-[52px] sm:px-6">
        ...
        <div className="flex flex-wrap items-center gap-9">
          <div className="min-w-[280px] flex-1 basis-[400px]">
            <span className="text-[11px] font-extrabold uppercase tracking-[0.12em] text-brand-deep">About us</span>
            ...
            <p className="mt-4 max-w-[520px] text-[15.5px] leading-relaxed text-neutral-500">
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <div className="rounded-2xl border border-neutral-200 px-4 py-3">
                <p className="text-[13px] font-extrabold text-neutral-900">Built in Nigeria</p>
                <p className="mt-0.5 text-xs text-neutral-400">For emerging markets</p>
              </div>
              <div className="rounded-2xl border border-neutral-200 px-4 py-3">
                <p className="text-[13px] font-extrabold text-neutral-900">Shaped by operators</p>
                <p className="mt-0.5 text-xs text-neutral-400">Retail managers and supervisors</p>
              </div>
            </div>
          </div>
        ...
        <div className="mt-9 grid gap-3.5 sm:grid-cols-2">
          {MISSION_CARDS.map((m) => (
            <article key={m.title} className="rounded-lg border border-neutral-200 bg-white p-5.5">
              ...
              <p className="mt-2 text-[13.5px] leading-relaxed text-neutral-500">{m.body}</p>
            </article>
          ))}
        </div>
        ...
        <section className="mt-9 flex flex-wrap gap-6">
          <div className="min-w-[280px] flex-1 basis-[400px]">
            <span className="text-[11px] font-extrabold uppercase tracking-[0.12em] text-brand-deep">Our story</span>
            <h2 className="mt-3 text-[28px] font-extrabold tracking-[-0.03em] text-neutral-900">
              We build what retail teams actually need.
            </h2>
            <p className="mt-3.5 text-sm leading-relaxed text-neutral-500">
          ...
          <ul className="flex min-w-[260px] flex-1 basis-[300px] flex-col gap-3.5 rounded-lg border border-[#F7DFD1] bg-[#FDF7F3] p-5.5">
            {BELIEFS.map((b) => (
              <li
                key={b}
                className="flex items-center gap-2.5 border-b border-[#F7E3D6] pb-3 text-sm font-bold text-neutral-900 last:border-b-0 last:pb-0"
              >
        ...
        <section className="mt-9">
          <span className="text-[11px] font-extrabold uppercase tracking-[0.12em] text-brand-deep">
          ...
          <div className="mt-4 grid gap-3.5 sm:grid-cols-2">
            {FOUNDERS.map((f) => (
              <article key={f.name} className="rounded-2xl border border-l-[3px] border-neutral-200 border-l-brand-500 bg-white px-5.5 py-5">
                ...
                <p className="mt-1 text-[13px] font-bold text-brand-deep">{f.role}</p>
                <p className="mt-2.5 text-[13.5px] leading-relaxed text-neutral-500">{f.body}</p>
              </article>
            ))}
          </div>
        </section>
